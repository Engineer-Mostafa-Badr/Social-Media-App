...
import 'package:social_media_app/data/repository/firebase_storge.dart';
...
class AddNewPostCubit extends Cubit<AddNewPostState> {
  ...
  AddNewPostCubit({required this.postsCubit, this.postEdit})
    : super(
        AddNewPostState(
          postContent: TextEditingController(text: postEdit?.contentPost),
  ...
  Future<void> pickedPostImages() async {
    ...
    if (result != null) {
      ...
      emit(state.copyWith(images: List.from(files)));
    }
  }
  ...
  Future<void> addPost(BuildContext context) async {
    ...
    FirebaseDataBase firebaseDataBase = FirebaseDataBase(FireBaseDatabaseApp());
    ...
    if (resultApi.isError) {
      ...
      emit(state.copyWith(dialogsType: DialogsType.successful));
    }
  }

  void restDialog() {
    emit(state.copyWith(dialogsType: DialogsType.init));
  }
}
